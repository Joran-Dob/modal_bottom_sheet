...
class _DefaultSheetScrollController extends StatelessWidget {
  const _DefaultSheetScrollController({Key? key, required this.child}) : super(key: key);
...
class SheetPosition extends ScrollPositionWithSingleContext {
  ...
  Future<void> animateTo(double to, {required Duration duration, required Curve curve}) {
    return super
        .animateTo(to.clamp(minScrollExtent, maxScrollExtent), duration: duration, curve: curve);
  }
  ...
  static double relativeOffsetFromPixels(
    ...
    final value =
        ((pixels - minScrollExtent) / (maxScrollExtent - minScrollExtent)).clamp(0.0, 1.0);
    return value;
  }
